#!/bin/bash

export GITHUB_USER="JarlPenguin"
export GITHUB_EMAIL="jarlpenguin@outlook.com"

export device="river"

export ROM="LineageOS"
export ROM_DIR="/mnt/data/jarl/lineage-22"
export ROM_VERSION="22.1"
export local_manifest_url="https://github.com/JarlPenguin/manifests/raw/master/lineage/22.x/sdm632.xml"
export manifest_url="https://github.com/LineageOS/android"
export rom_vendor_name="lineage"
export branch="lineage-22.1"
export bacon="bacon"
export buildtype=""
export clean="installclean"
export generate_incremental="false"
export upload_recovery="true"

export ccache="true"
export ccache_size="100"

export jenkins="false"

export release_repo="JarlPenguin/releases"

export timezone="UTC"
